...
Extracts functions, impl blocks, and closures.
"""
...
class RustChunker(BaseChunker):
    ...
    def chunk(
        ...
        """Extract Rust functions and impl blocks."""
        self._ensure_parser()
    ...
    def _extract_chunks(
        ...
        # Impl blocks - recurse into them with context
        elif node.type == "impl_item":
            impl_name = self._get_impl_name(node)
            new_context = f"impl {impl_name}" if impl_name else "impl"
        ...
        # Module - use as context
        elif node.type == "mod_item":
            mod_name = self._get_name(node)
            new_context = f"mod {mod_name}" if mod_name else context
    ...
    def _create_function_chunk(
        ...
        # Method if inside impl or trait
        chunk_type = "method" if context else "function"
    ...
    def _get_impl_name(self, node) -> Optional[str]:
